...
if (isset($_POST['saveNewComment']) && empty($_POST['escobar'])) {
    ...
    $formComment = Outils::cleanUpValues($_POST);

    // On récupère les valeurs des champs du formulaire
    $commentaire = $formComment['content'];

    // On valide le champ commentaire
    $FormErr = [];
    ...
    if (count($FormErr) === 0) {
        $comment = new Comment($formComment);
        if (!empty($conx)) {
        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    ...
                    <small>le <?= $res_post['datePost']; ?> par
                        <strong><?= $res_post['firstname']; ?> <?= $res_post['lastname']; ?></strong>.</small><br/>
                    <?php
            ...
            <div class="row mt-3">
                <div class="col">
                    <div class="card">
                        <div class="card-header">
                            <h3>Commentaire</h3>
                            <small>le <?= $comment['dateComment']; ?> par
                                <strong><?= $comment['firstname']; ?> <?= $comment['lastname']; ?></strong>.</small>
                        </div>
                        <div class="card-body">
                            <p class="card-text">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="detail.php?postid=<?= $res_post['postid'] ?>" method="post" autocomplete="off">
                        ...
                        <?php
                        $commentValidationClass = '';
                        if (isset($FormErr) && in_array('content', $FormErr)) {
                            $commentValidationClass = ' is-invalid';
                        }
                        ?>
                        <div class="form-group has-validation">
                            <label class="form-label" for="content">C'est ici : </label>
                            <textarea rows="4" class="form-control<?= $commentValidationClass; ?>" name="content" id="content"></textarea>
                        ...
                        <div class="form-group mt-3 mb-3 float-end">
                            <button type="submit" name="saveNewComment" class="btn btn-sm btn-primary">Enregistrer
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <div class="row mt-3 mb-3">
        <div class="col">
            <a href="<?= $_SESSION['default_url'] . '?blogid=' . $_SESSION['blogid']; ?>"
               class="btn btn-sm btn-primary">Retour à la liste</a>
        </div>
    </div>
</div>
